...
function App() {
  ...
  const allCompleted = todos.length > 0 && todos.every((t) => t.isCompleted === true);
    ...
    <div className="container">
      ...
      <header className="header">
        ...
        <div className="input-group">
          ...
          />
          <button type="submit" id="addBtn" className="btn-add" onClick={addTask}>
            Thêm
          </button>
        </div>
      </header>
            ...
            return (
              <div key={index} className="task-item" style={opacityStyle}>
                <input
                  ...
                  onChange={() => toggleTask(index)}
                />
                <span className={`task-text ${textClass}`}>{item.nameTask}</span>
